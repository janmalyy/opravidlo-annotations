...
if __name__ == "__main__":
    ...
    corpus_name = "cstenten23_mj2"     # cstenten_all_mj2, cstenten23_mj2
    koncovka = "a"
    target = f"l{koncovka}"
    if koncovka == "i":
        variants = ["y", "a", "o", ""]
        ...
        variants = ["i", "a", "o", ""]
    variants_weights = [1/4, 1/4, 1/4, 1/4]
    is_target_code = True
    ...
    podmet = '[tag="[NPA]..P1.*"]'
    podmet_sketch = '[tag="k[123].*nP.*c1.*"]'
    # in_between = '[word!="jsme|jste|by|byste|bychom" & tag!="Z.*"]'
    in_between = '[tag!="V[Bc].*" & tag!="Z.*"]'
    in_between_daleko = '[word!="." & tag!="Vp.*" & tag!="N..P1.*" & tag!="V[Bc].*"]'
    in_between_daleko_sketch = '[word!="." & tag!="k5.*mA.*" & tag!="k1.*c1.*" & word!="jsem|jsi|bych|bys|by"]'
    by = '[word="by"]'
    prisudek = f'[tag="Vp.P....R..A.*" & lemma!="být" & word=".*l{koncovka}"]'
    prisudek_sketch = f'[tag="k5.*mA.*" & lemma!="být" & word=".*l{koncovka}"]'
    not_podmet = '[tag!="[NP]..P1.*"]{3}'
    not_podmet_sketch = '[tag!="k[13].*c1.*"]'
    # query = podmet + in_between + in_between + by + in_between + in_between + prisudek
    # query = not_podmet + prisudek + in_between + '{0,3}' + podmet
    # query = prisudek + in_between_daleko + '{4,7}' + podmet
    # query = '[word="jsme"]'
    query =  not_podmet_sketch + '{5}' + prisudek_sketch + in_between_daleko_sketch + '{0,7}' + podmet_sketch
    ...
    number_of_concordances_to_fetch = 1500
    number_of_concordances_to_log = 50
