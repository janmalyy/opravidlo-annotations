...
if __name__ == "__main__":
    ...
    query = not_mi_word + '{2}' + mi_word + '{1}' + not_mi_word + mi_word + '{3}' + mi_noun
    # query = '[(tag="k2.*" | tag="k3.*nS.*" | tag="k4.*xO.*" | tag="kI.*" | lemma="tři|čtyři") & word=".*mi"]{2}[word=".*mi" & tag="k1.*nP.*c7.*"]'
    # query = '[word!=".*ma"]{2}[word=".*[^e][^j]ma" & tag="k[234].*nP.*c7.*"][word=".*ma" & lemma!="ruka|noha|oko|ucho" & tag="k1.*nP.*c7.*"]'
    ...
    number_of_concordances_to_fetch = 30
    number_of_concordances_to_log = 15
